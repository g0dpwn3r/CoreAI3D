...
private:
    ...
    std::vector<float> hidden_output; // Example: output after activation function of hidden layer
    ...
    bool safe2DIndex(const std::vector<std::vector<float>>& matrix, int i, int j);
...
public:
    CoreAI(int inputSize_, int layers_, int neurons_, int outputSize_, float min_, float max_);

    // Getters for external access
    std::vector<std::vector<float>>& getInput();
    std::vector<std::vector<float>>& getOutput(); // This should ideally be 'getTargets' if it stores original targets
    std::vector<std::vector<float>>& getResults(); // This is the output of the model
    ...
    void setOutput(const std::vector<std::vector<float>>& data); // If this is for original targets
    void setTarget(const std::vector<std::vector<float>>& data); // Clarifies setting target data
    ...
    void populateFields(int numInput, int numOutput); // Initializes weights, etc.
    std::vector<std::vector<float>> forward(const std::vector<std::vector<float>>& inputData);
    void train(const std::vector<std::vector<float>>& inputData, const std::vector<std::vector<float>>& targetData, double learningRate);
    void normalizeInput(float min_val, float max_val);
    void normalizeOutput(float min_val, float max_val); // This is likely normalizing targets
